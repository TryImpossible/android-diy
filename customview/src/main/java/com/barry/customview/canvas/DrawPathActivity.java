...
public class DrawPathActivity extends CanvasActivity {
    @Override
    void draw(Canvas canvas) {
        ...
        RectF arcRectF1 = new RectF(0, 0, deltaX, deltaY);
        path3.arcTo(arcRectF1, 270, 90); // 绘制圆弧
        pointList.add(new Point(deltaX, deltaY / 2));
        // 3.第三部分，绘制椭圆左下角的四分之一的弧线
        // 注意，我们此处调用了path的moveTo方法，将画笔的移动到我们下一处要绘制的arc的起点上
        path3.moveTo(deltaX * 1.5f, deltaY);
        RectF arcRectF2 = new RectF(deltaX, 0, deltaX * 2, deltaY);
        path3.arcTo(arcRectF2, 90, 90); //绘制圆弧
        ...
        paint.setStrokeWidth(10); // 将点的strokeWidth设置的比画path时要大
